#' Fit Bayesian Tensor Skew-t (BSTT) model with tensor spike-and-slab lasso (TSSL) prior to GAAD data
#' Lee et al. (2023+) A New Class of Skewed Tensor Distributions
#'
...
#' @param rho: posterior samples of correlation of each mode of tensor response (3 x n.save)
#' @param sigma.sq: posterior samples of variance parameter (b x n.save)
#' @param lam.est: posterior samples of skewness parameters (b x n.save)
#' @param B.est: posterior samples of tensor regression coefficients (t x s x b x p x n.save)
#' @param eta.est: posterior samples of common covariate effects (p x b x n.save)
#' @param alpha.est: posterior samples of sparsity elements that identify
#' different experience of elements (t x s x b x p x n.save)
#' @param nu: posterior samples of degree of freedom (1 x n.save)
#' @export
#'

BTST_TSSL <- function(Y,X,vecy, n.burn = 10, n.save = 100, thin = 1){
  ...
  sigma.sq.save <- matrix(NA, b, n.save)
  lam.est.save <- matrix(NA, b, n.save)
  eta.save <- matrix(NA, p*b, n.save)
  alpha.est.save <- array(NA, dim = c(t,s,b,p,n.save))
  ...
  library(magrittr); library(doParallel); library(TruncatedNormal);library(SSLASSO)
  ...
  #----------------------------------------------------------------------------------------
  # Update sigma.sq using Theorem 3.3 (marginal density of skewed tensor elliptical density)
  ...
  #-----------------------------------------------------------------------------------------------------------
  # Update skewness parameter lam.est using Theorem 3.3 (marginal density of skewed tensor elliptical density)
  ...
  # using Theorem 3.3 (marginal density of skewed tensor elliptical density)
  ...
  #------------------------------------------------------------------------------------
  # Update eta using Theorem 3.3 (marginal density of skewed tensor elliptical density)
  eta.est.update <- function(X,vecy,inv.Sigma,W,lam.est, t,s,p){
    ...
    eta.xy1 <- X%*%t(vecy)[,1:(t*s)]%*%inv.Sigma[1:(t*s),1:(t*s)]
    eta.xw1 <- X%*%t(W)[,1:(t*s)]%*%inv.Sigma[1:(t*s),1:(t*s)]*lam.est[1]
    eta.xy2 <- X%*%t(vecy)[,(t*s+1):(2*t*s)]%*%inv.Sigma[(t*s+1):(2*t*s),(t*s+1):(2*t*s)]
    eta.xw2 <- X%*%t(W)[,(t*s+1):(2*t*s)]%*%inv.Sigma[(t*s+1):(2*t*s),(t*s+1):(2*t*s)]*lam.est[1]
    eta.xx <- X%*%t(X)
    eta.A.inv1 <- solve(kron(eta.xx,inv.Sigma[1:(t*s),1:(t*s)]) + 0.1*diag(t*s*p) )
    eta.A.inv2 <- solve(kron(eta.xx,inv.Sigma[(t*s+1):(2*t*s),(t*s+1):(2*t*s)]) + 0.1*diag(t*s*p) )
    vec.eta1 <- MASS::mvrnorm(1, mu = eta.A.inv1%*%(as.vector(t(eta.xy1)) - as.vector(t(eta.xw1))), Sigma = eta.A.inv1, tol = 1e-3)
    vec.eta2 <- MASS::mvrnorm(1, mu = eta.A.inv2%*%(as.vector(t(eta.xy2)) - as.vector(t(eta.xw2))), Sigma = eta.A.inv2, tol = 1e-3)
    eta.est <- c(apply(mat(array(vec.eta1, dim = c(t,s,p)),3),1,mean),apply(mat(array(vec.eta2, dim = c(t,s,p)),3),1,mean))
  ...
  B.est.update <- function(X,vecy,W,lam.est, inv.Sigma, inv.R1, inv.R2, t,s,p, eta.est,alpha.est){

    B <- array(NA, dim = c(t,s,b,p)) ;

    for (i in 1:(t*s*b)){
      TSSL[[i]] <- SSLASSO(t(X), t(t(vecy[i,])), variance = "unknown")
    }

    for (i in 1:(t*s*b)){
      alpha[,i] <- c(TSSL[[i]]$intercept[,6],TSSL[[i]]$beta[2:6,6])
    }

    for (j in 1:p){
      alpha.est[,,1,j] <- array(alpha[j,1:(t*s)], dim = c(t,s));
    }

    for (j in 1:p){
      alpha.est[,,2,j] <- array(alpha[j,(t*s+1):(2*t*s)], dim = c(t,s));
    }

    for (i1 in 1:t){
      for (i2 in 1:s){
        for (j in 1:p){
          B[i1,i2,1,j] <- eta.est[j] + alpha.est[i1,i2,1,j]
        }}}
    ...
    for (i1 in 1:t){
      for (i2 in 1:s){
        for (j in 1:p){
          for (jj in (p+1):(2*p)){
          B[i1,i2,2,j] <- eta.est[jj] + alpha.est[i1,i2,2,j]
        }}}}

    B.est <- mat(B,4)

    return(list(B.est, alpha.est))
  ...
  lam.est <- c(1,1)
  TSSL <- list()
  alpha <- matrix(NA, p, t*s*b)
  alpha.est <- array(0.001, dim = c(t,s,b,p))
  nu = 4
  ...
  for (i in 1:(n.burn + n.save*thin)) { #}
    ...
    B.result =  B.est.update(X,vecy,W,lam.est, inv.Sigma, inv.R1, inv.R2, t,s,p, eta.est,alpha.est)
    B.est = B.result[[1]]; alpha.est = B.result[[2]]
    a_delta <- nu/2 + t*s*b;
    for (l in 1:290){
  ...
  result = list(rho.save, sigma.sq.save, lam.est.save, B.est.save,eta.save,alpha.save,nu.save)
  names(result) = c('rho', 'sigma.sq','lam.est','B.est','eta.est','alpha.est','nu')

  return(result)

} # end BTST_TSSL function
