---
title: "A New Class of Tensor Skew Distributions"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{A New Class of Tensor Skew Distributions}
...
- Lee, I., Sinha, D., Mai, Q. & Bandyopadhyay, D. (2023+). *A New Class of Tensor Skew Distributions.* [[Github]](https://github.com/InkooLee/BSTT)
...
of covariate effects, we use $\beta_{i_1i_2i_3j} = \eta_{i_3j} + \alpha_{i_1i_2i_3j} + \epsilon_{i_1i_2i_3j}$ to split 
...
biomarker $i_3$ from site $i_2$ of tooth $i_1$, and extremely small values $\epsilon_{i_1i_2i_3j}$.
...
When $\gamma \ll \gamma_0$, tensor spike-and-slab lasso prior closely retrieve to tensor spike-and-slab prior. 
...
## GAAD data analysis using `BTST` package
We fit the BTST (Bayesian tensor skew-$t$) model with tensor spike-and-slab lasso (TSSL)/Tensor Normal (TN) prior.
...
BTST_TSSL <- BTST_TSSL(Y,X,vecy, n.burn = 100, n.save = 1000, thin = 5)
BTST_TN <- BTST_TN(Y,X,vecy, n.burn = 100, n.save = 1000, thin = 5)
```
